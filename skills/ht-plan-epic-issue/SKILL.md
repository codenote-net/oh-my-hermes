...
Do not implement any sub-issue, create a branch or pull request, invoke `ht-issue-loop`, merge,
...
- be implementable by one `ht-issue-loop` run without requiring hidden repository knowledge;
...
reviews, or mutate the parent. Those are `ht-issue-loop` orchestration responsibilities.
...
- that each sub-issue URL is ready for an independent `ht-issue-loop` run when its prerequisites
  are satisfied.
