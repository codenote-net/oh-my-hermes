...
8. For a new run, create `ht/issue-<number>-<short-topic>` from the latest remote default tip and
...
4. Append the idempotency marker
   `<!-- ht-issue-loop-fix-limit:<current-head-sha> -->`. Inspect comments on the selected target
...
5. Post exactly one comment with this shape, substituting the complete progress summary:
   ...
   ```text
   @<login> The ht-issue-loop fix limit was reached after 10/10 fix attempts.
   ...
   <!-- ht-issue-loop-fix-limit:<current-head-sha> -->
   ```
