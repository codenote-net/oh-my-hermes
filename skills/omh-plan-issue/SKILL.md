...
The successful output is the exact Issue URL accepted by `omh-issue-loop`. Do not implement the
plan, change repository files, create a branch or pull request, or invoke `omh-issue-loop` from this
skill.
...
- Keep acceptance criteria outcome-oriented so `omh-issue-loop` can prove them. Do not include
  instructions that conflict with `omh-issue-loop` ownership, such as asking the implementation
...
Use the same fixed model choices as `omh-issue-loop` while enforcing a stricter read-only review
boundary:
...
- that the URL is ready to pass to `omh-issue-loop`.
